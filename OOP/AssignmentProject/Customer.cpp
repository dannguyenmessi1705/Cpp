...
public:
    ...
    bool IsEmpty()
    { // Ham check Danh sach co rong hay khong
        if (this->pHead == NULL)
            return true;
        else
            return false;
...
void TimDoDai(int &lenid, int &lenname, int &lenaddress, int &lenemail, int &lenphone, int &lenrate, int &lenbill)
{
    ...
    else
    {
        ...
        { // Bat dau tu sau tu "ID" de tim khoang trong do dai
            if (line[i] != ' ')
                break;
            else
                lenid++;
        ...
        for (int i = lenid + lenname; i <= line.length(); i++)
        { // Bat dau tu sau tu "NAME" de tim khoang trong do dai
            if (line[i] != ' ')
                break;
            else
                lenname++;
        ...
        for (int i = lenid + lenname + lenaddress; i <= line.length(); i++)
        { // Bat dau tu sau tu "ADDRESS" de tim khoang trong do dai
            if (line[i] != ' ')
                break;
            else
                lenaddress++;
        ...
        for (int i = lenid + lenname + lenemail + lenaddress; i <= line.length(); i++)
        { // Bat dau tu sau tu "EMAIL" de tim khoang trong do dai
            if (line[i] != ' ')
                break;
            else
                lenemail++;
        ...
        for (int i = lenid + lenname + lenemail + lenphone + lenaddress; i <= line.length(); i++)
        { // Bat dau tu sau tu "PHONE" de tim khoang trong do dai
            if (line[i] != ' ')
                break;
            else
                lenphone++;
        ...
        for (int i = lenid + lenname + lenemail + lenphone + lenaddress + lenrate; i <= line.length(); i++)
        { // Bat dau tu sau tu "RATE" de tim khoang trong do dai
            if (line[i] != ' ')
                break;
            else
                lenrate++;
        ...
        for (int i = lenid + lenname + lenemail + lenphone + lenaddress + lenrate + lenbill; i <= line.length(); i++)
        { // Bat dau tu sau tu "BILL" de tim khoang trong do dai
            if (line[i] != ' ')
                break;
            else
                lenbill++;
...
void DocInfoTuFile(ifstream &FileIn, Customer &x)
{
    ...
    for (int i = sbill.length() - 1; i >= 0; i--)
    {
        if (sbill[i] != ' ')
            ...
            sbill.erase(sbill.end() - 1);
    }
    bill = 0;
    int mu = 0;
    for (int i = sbill.length() - 1; i >= 0; i--)
    { // Chuyen string sang so
        if (tmp[i] >= '0' && tmp[i] <= '9')
...
{
    while (!FileIn.eof())
    { // Doc file cho toi khi den cuoi dong
        Customer x;
...
bool KiemTraDauVao(LIST l, Customer x)
{
    ...
    if (checkemail == 0 && checkphone == 0)
        return true;
    else if (checkemail > 0 && checkphone > 0)
        cout << "\t\tEmail va So dien thoai vua nhap da ton tai\n";
    else if (checkemail == 0 && checkphone > 0)
        cout << "\t\tSo dien thoai vua nhap da ton tai\n";
    else
        cout << "\t\tEmail vua nhap da ton tai\n";
    return false;
}
void Customer::NhapThongtin()
{
    ...
    do
    {
        ...
        if (name.length() > 25 || name.length() == 0)
            cout << "\t\tTen qua dai hoac khong hop le, vui long nhap lai(25 ky tu): ";
    ...
    {
        getline(cin, address);
        if (address.length() > 18 || address.length() == 0)
            cout << "\t\tDia chi qua dai hoac khong hop le, vui long nhap lai(18 ky tu): ";
    ...
    do
    {
        ...
        if (email.length() > 30 || email.length() == 0)
            cout << "\t\tEmail qua dai hoac khong hop le, vui long nhap lai(30 ky tu): ";
    ...
    do
    {
        ...
        if (phone.length() > 12 || phone.length() == 0)
            cout << "\t\tSo dien thoai khong hop le, vui long nhap lai(12 so): ";
...
void XoaInfo(LIST &l, NODE *q)
{
    ...
    while (strcmp(l.pHead->data.getEmail().c_str(), q->data.getEmail().c_str()) == 0 && l.pHead->pNext != NULL)
    { // Phong truong hop phan tu tiep theo se thanh pHead lai chinh bang node q
        l.XoaDau();
    }
    ...
    while (k != NULL)
    {
        ...
        if (strcmp(k->data.getEmail().c_str(), q->data.getEmail().c_str()) == 0 && k->pNext == NULL)
        { // Neu phan tu node q can xoa o vi tri cuoi cung
            g->pNext = k->pNext;
            l.pTail = g;
            delete k;
            return;
...
void TotalRate(ifstream &FileIn, LIST l)
{
    ...
    for (NODE *k = l.pHead; k != NULL; k = k->pNext)
    {
        if (k->data.getRate() == 0)
            rate0++;
        else if (k->data.getRate() == 1)
            rate1++;
        else if (k->data.getRate() == 2)
            rate2++;
        else if (k->data.getRate() == 3)
            rate3++;
        else if (k->data.getRate() == 4)
            rate4++;
        else
            rate5++;
    }
